...
#    storage_options={})
    ...
    def upload_do():
        ...
        for f in fname:
            if f and allow_f(f.filename):
                ...
                new_fname = r'/web/static/up_files/' + t + f.filename
                f.save(new_fname)
            elif f and allow_f(f.filename) == 0:
                return '不允许的上传文件格式'
            else:
                return '请上传文件'
        return 'you have already upload the file!'
    ...
    def pics():
        ...
        if (request.args.get("height") != None) and request.args.get("width") != None:
            ...
            image = cv2.imread(f"/web/static/pic/立绘_{get_pic}_1.png")
            ...
            with open(f"/web/static/pic/立绘_{get_pic}_1.png", 'rb') as f:
                fimage = f.read()
